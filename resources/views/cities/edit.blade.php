        ...
        <div class="card mx-auto" style="width: 100%;">
            <div class="card-header">{{ __('Update City') }}
                <form action="{{ route('cities.destroy', $city->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button class="float-right btn btn-danger" title="Delete {{ $city->name }}"><i
                            class="fa fa-trash fa-fw"></i></button>
                </form>
                <a href="{{ route('cities.index') }}" class="btn btn-primary float-right mx-2">Back</a>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('cities.update', $city->id) }}">
                    ...
                    @method('PUT')
                    <div class="row mb-3">
                        <label for="states_id" class="col-md-4 col-form-label text-md-end">{{ __('State') }}</label>
